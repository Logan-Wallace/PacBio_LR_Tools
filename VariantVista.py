...
'''The purpose of this program is to concatenate variant call files from multiple individuals into a collection of bed files that can be loaded to a custom track on the ucsc genome browser and shared with colleagues.'''
...
logging.basicConfig(filename='VariantVista.log', filemode='w', format='%(name)s - %(levelname)s - %(message)s', level=logging.DEBUG)

# Set some command line arguments
parser = argparse.ArgumentParser(
                    prog='VariantVista.py',
...
parser.add_argument("-w", "--window", help = "The window size for collapsing variant calls of the same SVTYPE. Defaults to 10bp.", type = int, default = 10)

parser.add_argument("-r", "--records", help = "The number of recent records to check the most recent record against. Defaults to 5.", type = int, default = 5)
...
parser.add_argument("-p", "--percent_cutoff", help = "Variant percent of reads below which, variants will be tossed out. Defaults to 5 percent.", type = float, default = 5)

parser.add_argument("-r", "--read_cutoff", help = "Number of reads below which, variants will be tossed out. Defaults to 5 reads.", type = int, default = 10)
...
window = args.window
percent_cutoff = (args.cutoff / 100)
...
def get_record(line):
    ...
    if variant_type == 'BND':
        ...
        record = ""
        variant_type = ""
    # If the record has met the cutoffs, return the record and the variant type
    if ALT >= read_cutoff and fusion_perc >= percent_cutoff:    
        return record, variant_type
    else:
        logging.infor("Variant did not meet the cutoffs: {}".format(record))
        record = ""
        return record, variant_type
